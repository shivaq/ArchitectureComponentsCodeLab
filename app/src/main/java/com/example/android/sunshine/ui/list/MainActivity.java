...
public class MainActivity extends AppCompatActivity implements
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    MainVMFactory factory = InjectorUtils
        .provideMainActivityViewModelFactory(this.getApplicationContext());
    viewModel = ViewModelProviders.of(this, factory).get(MainActivityViewModel.class);

    viewModel.getCurrentWeatherForecasts().observe(this, weatherEntryList -> {
      if (weatherEntryList != null) {
        swapForecast(weatherEntryList);
      }
    });
  }

  private void swapForecast(List<ListWeatherEntry> weatherEntryList) {
    mForecastAdapter.swapForecast(weatherEntryList);
    if (mPosition == RecyclerView.NO_POSITION) {
      mPosition = 0;
    }
    mRecyclerView.smoothScrollToPosition(mPosition);
    if (weatherEntryList != null && weatherEntryList.size() != 0) {
      showWeatherDataView();
    } else {
      showLoading();
    }
