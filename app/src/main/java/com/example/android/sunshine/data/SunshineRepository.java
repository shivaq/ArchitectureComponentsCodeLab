...
public class SunshineRepository {
  ...
  private SunshineRepository(WeatherDao weatherDao,
    ...
    // リソースからのデータをLiveData にして
    LiveData<WeatherEntry[]> networkData = mWeatherNetworkDataSource.getCurrentWeatherForecasts();
    // Observe する
    // observeForever は observe と違い、常にActive。
    // よって、Lifecycle とは紐付いていない。
    // なぜここで使うのか？
    // Repository も リモートのデータソースも、UI のLifecycle と関係ない。
    // アプリ全体のLifecycle が続いている限り、存在し続けるから。
